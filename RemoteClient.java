...
public class RemoteClient extends JFrame implements ActionListener, Runnable, Serializable {
	Robot r;
	...
	int port_to_host_number = 12561;
	int screen_port = 12562;
	// 공유 키
	...
	public RemoteClient() {
		...
		top_label3 = new JLabel("로그인 하면 코드가 생성됩니다.");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("connect")) {
			...
			if (connectCheck) {
				...
				Thread rScreen = new Thread(new RemoteScreen());
				rScreen.start();
			} else {
				...
				if (in_msg != null) {
					if (in_msg.startsWith("#press#") && shareKey.equals(in_msg.split(":")[1])
						...
						Thread dt = new Thread(new Download(in_msg.split(":")[3], in_msg.split(":")[4],
								Integer.valueOf(in_msg.split(":")[5])));
						dt.start();
					}
				} else {
					break;
		...
		public void run() {
			try {
				...
				out.println("#p2p#" + ":" + connectKey + ":" + connectName + ":" + send_server_address + ":"
						+ sendFile.getName() + ":" + sendFile.length());
	...
	class Download extends JFrame implements Runnable {
		JProgressBar downloadBar;
		JButton downEnterBtn;
		...
		public Download(String p2p_address, String receiveFileName, int fileSize) {
			super(receiveFileName + "다운로드");
			this.setSize(300, 80);
			...
			downloadBar.setBackground(Color.white);
			downloadBar.setFont(new Font("Dialog", Font.BOLD, 12));
			downloadBar.setForeground(Color.blue);
			...
			if (savePath.equals("C:\\")) {
				...
				download_filename = savePath + "\\" + receiveFileName;
			try {
				socket = new Socket(p2p_address, 12563);
		...
		public void run() {
			...
			try {
				...
				while ((c = in.read()) != -1) {
					size += c;
					fos.write(c);
					downloadBar.setValue((int) ((size * 100 / fileSize) / 128));
					downloadBar.getString();
				}
				...
				if (connectOn) {
					try {
						...
						while (bout != null) {
							Thread.sleep(150);
							bufImg = capture();
							ImageIO.write((BufferedImage) bufImg, "png", bout);
							bout.flush();
						}
						bout.close();
						sendSocket.close();
...
// 원격 화면
	class RemoteScreen extends JFrame
	...
	class ReceiveServer implements Runnable {
		...
		public void run() {
			try {
				...
				while (true) {
					bufImg = ImageIO.read(bin);
					if (bufImg != null) {
						RemoteScreen.receiveScreenPanel.getGraphics().drawImage(bufImg, 0, 0,
								RemoteScreen.receiveScreenPanel.getWidth(), RemoteScreen.receiveScreenPanel.getHeight(),
								RemoteScreen.receiveScreenPanel);
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
	...
	class Phone extends JFrame implements Runnable {
		...
		public class Connect extends Thread {
			...
			Connect(Phone p) {
				this.p = p;
				try {
					socket = new Socket(send_server_address, 12564);
